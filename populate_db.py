...
serverOne = Server.query.filter_by(name='serverOne').first()
serverTwo = Server.query.filter_by(name='serverTwo').first()
messageOne = Message(payload="Hello World", sender_id=nico.id, receiver_id=luca.id, server_id=serverOne.id, timestamp=time())
messageTwo = Message(payload="Hello World back!", sender_id=luca.id, receiver_id=nico.id, server_id=serverOne.id, timestamp=time())
messageThree = Message(payload="Private Message", sender_id=luca.id, receiver_id=nico.id, server_id=serverTwo.id, timestamp=time())
